...
router.get('/getByDateRange', verify, (req, res) => {
    ...
    var result = JsonData.alarms.filter(item => new Date(item.timestamp).getTime() > new Date(start_time).getTime() && new Date(item.timestamp) < new Date(end_time).getTime())
    const response = result.slice(startIndex, endIndex);
    console.log("ReponseData", response);
    res.json(response);
    // console.log("JsonData",JsonData.alarms.filter(item=> new Date(item.timestamp).getTime() > new Date(start_time).getTime() && new Date(item.timestamp) < new Date(end_time).getTime()));
